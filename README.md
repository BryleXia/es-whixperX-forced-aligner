...
> 把西班牙语字幕文件的时间戳准确对齐到音频上。提供两套互补方案，覆盖不同录音质量场景。方案 A 在 5 段纪录片音频（390 条字幕，约 56 分钟）上经过人工逐句审听，**0 条错误**。
...
**API 配置：** 需要在脚本开头填入 LLM API Key（默认使用 AiHubMix 中转，兼容 OpenAI SDK 格式）。
...
- 两个方案均在西班牙语上验证，法语、俄语支持已规划（wav2vec2 模型可用，需切换语言代码）
